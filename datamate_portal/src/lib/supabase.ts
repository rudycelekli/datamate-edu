...
// Desafio schema client (server-side, service role)
// eslint-disable-next-line @typescript-eslint/no-explicit-any
let _desafioClient: any = null;

export function getDesafioClient() {
  if (_desafioClient) return _desafioClient;
  const url = process.env.SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) {
    throw new Error("SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required (server-only)");
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _desafioClient = createClient(url, key, { db: { schema: "desafio" } } as any);
  return _desafioClient;
}
